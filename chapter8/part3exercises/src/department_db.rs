...
impl DepartmentDb {
    ...
    pub fn get_department(&self, dept_name: &NotEmptyString) -> Option<Vec<NotEmptyString>> {
        let department = self.departments.get(dept_name)?;
        Some(department.clone())
    }

    pub fn add_person_to_department(&mut self, person: NotEmptyString, department: NotEmptyString) {
        self.departments
            .entry(department)
            .or_insert(Vec::new())
            .push(person);
    ...
    pub fn add_employee(&mut self, employee: Employee) {
        self.add_person_to_department(employee.name, employee.department);
    }

    pub fn get_all_employees(&self) -> Vec<Employee> {
        let mut employees = self.create_employees_vec();
        employees.sort_by(|a, b| a.name.cmp(&b.name));
        employees
    }

    fn create_employees_vec(&self) -> Vec<Employee> {
